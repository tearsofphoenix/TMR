...
function tmr_log(msg) {
    ...
    if (logger) { console.log(arguments) }
}
...
function tmr_check_arg(arg, type, force, idx) {
    var prefix = ['first', 'second', 'third', 'forth'];
    //force check
    if (force) {
        if (tmr_typename(arg) != type) {
            tmr_throw_error(prefix[idx] + 'argument should be ' + type + '!');
        }
    }else{
        if (arg && tmr_typename(arg) != type) {
            tmr_throw_error(prefix[idx] + 'argument should be ' + type + '!');
        }
    }
}
...
function TMRInherit(Self, Super) {
    ...
    _NestClass.prototype = Super.prototype;
    Self.me = Super.prototype;
    Self.prototype = new _NestClass;
    Self.superCall = function (a, c, f) {
